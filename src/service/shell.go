...
func sshSession (c ContainerDigest) {
    ...
    if path, ok := SshKeys[c.Instance.KeyName + ".pem"]; ok {

        pk, err := ioutil.ReadFile(path)
        if err != nil {
            log.WithError(err).WithField("path", path).Fatal("Could not read .pem")
        }
        signer, err := ssh.ParsePrivateKey(pk)
        if err != nil {
            log.WithError(err).WithField("path", path).Fatal("Could not parse .pem")
        }
        ...
        in <- fmt.Sprintf("docker exec -it %s bash", containerId)
        fmt.Print(<-out)
        ...
        log.WithField("key", c.Instance.KeyName).Info("Could not find SSH key")
    }
}
